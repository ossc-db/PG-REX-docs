...
本章では、コマンドを直接実行して運用する手順を説明します。PG-REX運用補助ツールを用いて運用する場合は、『起動と停止』の章および『メンテナンス時の対応』の章を参照してください。
...
(8) 初回起動時、もしくは新しいリソース定義xmlファイルを反映させる場合、リソース定義xmlファイルを反映させます。
  ...
  ------------------------------------------------------------------------
  [# pcs cluster cib\-push PG\-REX13_pm_pcsgen_env.xml]{custom-style="Verbatim Char"}\
